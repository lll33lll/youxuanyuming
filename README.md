...
| 域名 | IP 来源 | 说明 |
| --- | --- | --- |
| `cf.223226.xyz` | [saas.sin.fan](https://saas.sin.fan) SIN 优选域名**多视角全量解析**（**临时测试模式**：阿里/腾讯/Google/Cloudflare 四路 DoH 多轮查询合并，GeoDNS 各地区/轮换记录全收；另外三个源暂移除） | 官方网段·精选，每 3 小时 |
...
- 新增「多视角 DNS 采集」源型（dns-multi 型源）：阿里/腾讯/Google/Cloudflare 四路 DoH 多轮查询合并，GeoDNS 优选域名的各地区/轮换记录全收
- 同步频率拆分：官方池每 3 小时、反代池每 1.5 小时（同一 workflow 两个 cron 批次，按触发的 cron 区分范围）
- 采集增加「源大面积异常」守卫：可用源少于 1/4 或结果少于 10 个时不写文件，防止网络故障时把池子砍残
- `cf` 域名源定型：ipTop10 + SIN 优选域名 + CloudFlareYes 电信 + 微测网；当前处于**临时测试模式**（只用 dns-multi:saas.sin.fan，其余三个暂移除，测试完恢复）
